...
/** A default-export factory that registers one workflow (+ matching agent). */
function extFile(workflow: string, opts: { agent?: string; description?: string } = {}): string {
  const ag = opts.agent ?? `${workflow}-agent`;
  const desc = opts.description ? `description: ${JSON.stringify(opts.description)}, ` : "";
  return [
    "export default function (autosk) {",
    `  autosk.registerWorkflow({ name: ${JSON.stringify(workflow)}, ${desc}firstStep: "do", steps: { do: { agent: ${JSON.stringify(ag)} } } });`,
    `  autosk.registerAgent({ name: ${JSON.stringify(ag)}, async onRun() {} });`,
    "}",
    "",
  ].join("\n");
}
...
describe("extension loader — deterministic merge / priority", () => {
  ...
  test("project beats global beats npm on a name collision; losers are diagnostics", async () => {
    write(join(proj, ".autosk/extensions/p.ts"), extFile("shared", { agent: "ap", description: "from-project" }));
    write(join(home, ".autosk/extensions/g.ts"), extFile("shared", { agent: "ag", description: "from-global" }));
    ...
    write(join(home, ".autosk/packages/node_modules/@s/pkg/i.ts"), extFile("shared", { agent: "apkg", description: "from-pkg" }));
    ...
    // The losing agents (different names) still register — only the dup name was skipped.
    expect(registry.agentNames()).toEqual(["ag", "ap", "apkg"]);
  });

  test("within a directory, sorted filename order decides the winner", async () => {
    write(join(proj, ".autosk/extensions/01-first.ts"), extFile("dup", { agent: "first", description: "first" }));
    write(join(proj, ".autosk/extensions/02-second.ts"), extFile("dup", { agent: "second", description: "second" }));
...
describe("extension loader — error isolation (daemon stays up)", () => {
  ...
  test("a throwing factory and a duplicate name surface in diagnostics; the rest register", async () => {
    ...
    write(ext("good-a.ts"), extFile("alpha", { agent: "a1" }));
    write(ext("good-b.ts"), extFile("beta", { agent: "b1" }));
    write(ext("nodefault.ts"), "export const notAFactory = 42;\n");
    write(ext("throws.ts"), 'export default function () { throw new Error("kaboom"); }\n');
    // zdup registers the duplicate workflow "alpha" but a fresh agent "a2".
    write(
      ext("zdup.ts"),
      [
        "export default function (autosk) {",
        '  autosk.registerWorkflow({ name: "alpha", firstStep: "do", steps: { do: { agent: "a2" } } });',
        '  autosk.registerAgent({ name: "a2", async onRun() {} });',
        "}",
        "",
      ].join("\n"),
    );
    ...
    // The duplicate extension's OTHER registration (agent a2) still landed.
    expect(registry.agentNames()).toEqual(["a1", "a2", "b1"]);
