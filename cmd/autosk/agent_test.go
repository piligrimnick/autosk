...
// package, exercised against a direct doltlite store) now lives behind the
// daemon: the write verbs pass `caller`/`author` and autoskd ensures the row.
// That behaviour is covered by the autosk-core verb tests; here we only verify
// the client-side name resolution.
