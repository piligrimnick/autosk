 ...
 *
 * The autosk daemon is auto-spawned by the CLI on first use, and the project is
 * resolved from the child's cwd (walk-up to the nearest `.autosk/`) — or from
 * `$AUTOSK_CWD` when set, which the autosk agent runtime points at the real
 * project root so these tools work even inside an isolated worktree.
 *
 * `pi.exec` swallows ENOENT into `code=1 / stdout="" / stderr=""`, which
 * collides with a genuine autosk failure that happens to produce no stderr. We
 * use the empty-result shape as a "missing binary" heuristic.
 */
	...
	if (result.code !== 0) {
		if (looksLikeMissingBinary(result)) {
			throw new AutoskCliError(
				...
				action,
				"missing_binary",
				"`autosk` binary not found on PATH. Build it (`make build`) and put ./bin on PATH, or symlink ./bin/autosk into /usr/local/bin.",
