...
import { buildCommentAddArgv, buildCommentListArgv, requireId } from "./argv.ts";
...
const CommentArgsSchema = Type.Partial(
	Type.Object({
		task_id: Type.String({
			description: "Target task id (e.g. ask-a1b2c3). Required for add and list.",
		}),
...
const TOOL_PROMPT_SNIPPET = "Append / read durable comments on autosk tasks via autosk_comment.";
...
const TOOL_PROMPT_GUIDELINES = [
	...
	"To correct a previous `autosk_comment` add, append a new comment, never try to edit.",
] as const;
...
async function executeCommentTool(
	...
	try {
		if (action === "add") {
			...
			const comment = await runAdd(pi, taskId, args, runOptions);
			const details: AutoskDetails = { kind: "comment", domain: DOMAIN, action, comment };
			return {
				content: [{ type: "text" as const, text: formatAddText(taskId, comment) }],
				details,
				isError: false,
			};
		}
...
async function runAdd(
	...
	const argv = buildCommentAddArgv(taskId, args.text ?? "", args.author);
...
function requireTaskId(action: CommentAction, args: CommentArgs): string {
	return requireId(DOMAIN, action, args.task_id);
}
...
function renderCommentResult(result: ToolResult, _options: unknown, theme: ToolTheme): Component {
...
function renderSingle(c: Comment, theme: ToolTheme): string {
	const head = `${theme.bold(theme.fg("accent", `[${c.id}]`))} ${theme.fg("muted", c.author)} ${theme.fg("dim", c.created_at)}`;
