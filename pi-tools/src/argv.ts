/**
 * Pure `autosk` argv builders + validation, split out from the tool modules so
 * they carry NO pi runtime dependency (only `import type` from the SDK, which is
 * erased) and can be unit-tested standalone. Flag names mirror the v2 CLI:
