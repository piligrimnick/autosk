...
// cleared current_step_id on terminal status (done|cancelled) so the
// CHECK in 001_init.sql stayed satisfied, while lazy's
...
// human_feedback with a non-null current_step_id (the typical
...
//     CHECK in 001_init.sql stays valid for tasks that were in
//     human_feedback or in_workflow.
...
//   - Reopen rejects tasks that are not in {done, cancelled}.
//   - SetStatus rejects status edits on in_workflow tasks: those
...
// Cancel flips the task to status='cancelled' and clears current_step_id.
...
func Cancel(ctx context.Context, s store.Store, id string, opts Options) (store.Task, error) {
	return setTerminal(ctx, s, id, store.StatusCancelled, opts)
}

// Reopen flips a done|cancelled task back to status='new'.
//
// Rejects tasks that aren't currently done|cancelled — there is no
// well-defined "reopen" for a new / in_workflow / human_feedback task
...
func Reopen(ctx context.Context, s store.Store, id string) (store.Task, error) {
	...
	if cur.Status != store.StatusDone && cur.Status != store.StatusCancelled {
		return store.Task{}, fmt.Errorf("cannot reopen task in status %q (only done|cancelled)", cur.Status)
	}
...
// Rejects in_workflow as a target AND as a source: workflow lifecycle
...
// target on a done|cancelled task is delegated to Reopen. Any other
// shape (e.g. human_feedback) is a plain TaskPatch{Status: …}.
func SetStatus(ctx context.Context, s store.Store, id string, target store.Status, opts Options) (store.Task, error) {
	...
	if target == store.StatusInWorkflow {
		return store.Task{}, errors.New("refusing to set status='in_workflow' directly; use `autosk enroll` or let the workflow engine advance it")
	}
	cur, err := s.GetTask(ctx, id)
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
	...
	if cur.Status == store.StatusInWorkflow {
		return store.Task{}, fmt.Errorf("refusing to change status on an in_workflow task; use `autosk done|cancel` or let the workflow engine advance it")
	}
	switch target {
	...
	case store.StatusNew:
		...
		// source was human_feedback with a step pointer (CHECK guard).
		if cur.Status == store.StatusDone || cur.Status == store.StatusCancelled {
	...
	// when the source had one — moving from human_feedback (which
	// allows a non-null step) to anything other than human_feedback
	// or in_workflow trips the CHECK otherwise.
	if cur.CurrentStepID != "" && target != store.StatusHumanFeedback {
		empty := ""
