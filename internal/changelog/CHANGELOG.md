...
- **gui:** the Sessions panel now renders each session as a lazy-mode-style two-line row — `<work-time>  job-xxxxxx  ask-xxxxxx  STATUS` over an indented `workflow-name:step-name` — dropping the leading status dot (the right-edge status chip already carries that signal, and running sessions keep a subtle pulse on the chip). The job id, task id, workflow name, and step name use lazy-mode's entity hues (magenta / blue / yellow / purple), and the left column shows pi's actual work time (started→finished, or →now while running) instead of a created-at "… ago" — and, like lazy-mode, it now counts up live (a 1s tick re-renders while any session is still running). The matching status dot in the session-transcript header is likewise gone (the chip beside the job id covers it).
...
- **gui:** the Tasks panel now renders each task as a single lazy-mode-style line — `P2  ask-xxxxxx  Title…  STATUS` — with the task id in lazy's blue hue, the title flex-growing (ellipsis-truncated) to fill the middle, and the status chip magnetised to the right edge. The per-task actions menu now opens as a **native OS context menu** on right-click of the row (the old hover/selection kebab button is gone, so the status chip is never occluded). Tasks are a single flat list sorted by most-recently-updated first instead of being grouped by status.
