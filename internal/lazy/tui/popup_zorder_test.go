...
// TestZOrder_SessionInputCreatedDuringPopup_PopupStaysOnTop pins the
// SUBSTANTIVE regression review flagged: when winSessionInput is
// created mid-popup (e.g. a queued session promotes to running while
...
func TestZOrder_SessionInputCreatedDuringPopup_PopupStaysOnTop(t *testing.T) {
	gu := newHeadlessGui(t, 120, 40)
	gu.ds = &refreshFakeDS{}

	// Step 1: seed a TERMINAL session (no input overlay yet) and
	// drive a layout pass to populate g.views with the dashboard
	// views.
	gu.st.withLock(func() {
		gu.st.sessions = []datasource.Session{{
			ID: "session-A", Status: "done",
		}}
	...
	if viewIndexInGui(gu, winSessionInput) != -1 {
		t.Fatalf("setup: winSessionInput should not exist before the promotion")
	}
	...
	// Step 3: flip the session to non-terminal (e.g. it was queued
	// and just started running). The next layout pass creates
	// winSessionInput because isSessionLive now returns true.
	gu.st.withLock(func() {
	...
	inputIdx := viewIndexInGui(gu, winSessionInput)
	popupIdx = viewIndexInGui(gu, winPopupMenu)
	if inputIdx == -1 {
		t.Fatalf("winSessionInput should exist after the session promotion")
	}
	if popupIdx == -1 {
